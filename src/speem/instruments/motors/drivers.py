...
class Automation1Driver(MotorDriver):
    ...
    def enable(self) -> None:
        if self.faults:
            warn(
                f"Axis {self.axis} has faults: {self.faults}. Clearing them and"
                "enabling the axis."
            )
    ...
    async def _check_faults(
        ...
        elif AxisFault.PositionErrorFault in faults:
            self._fault_and_error_commands.faultacknowledge(self.axis)
            self.enable()
            warn(
                f"Axis {self.axis} has a PositionErrorFault. Clearing it so you can try"
                "again."
            )
        else:
            raise exception
    ...
    async def move_relative(self, distance: float, speed: float = None) -> None:
        ...
        try:
            ...
            await self._check_faults(e, self.position + distance)
    ...
    async def move_absolute(self, position: float, speed: float = None) -> None:
        ...
        try:
            ...
            await self._check_faults(e, position)
