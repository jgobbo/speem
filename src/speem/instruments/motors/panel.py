...
from pylucam import LucamCamera, LucamError, LucamErrorCode
...
if TYPE_CHECKING:
    ...
    from daquiri.widgets import ComboBox, Subjective, PushButton, NumericEdit, Slider
...
class MotionPanel(BasicInstrumentPanel):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.sample_loading_camera = self.speem_camera
    ...
    def update_camera_view(
        ...
        if self.camera_update == CameraUpdate.GAIN:
            ...
            # except LucamError(LucamErrorCode.Timeout):
            # warning("Camera timed out while trying to take a frame.")
            except Exception as e:
                warning(f"Camera failed to take a frame with error: {e}")
